...
1. 研究者选择房间和词表。
2. Ollama 为全部目标词生成一个连续英文故事。
3. 每个西语词使用一张固定的本地图片，并放在稳定的家具锚点上。
4. 参与者进入房间，自主学习 20 分钟。
5. 完成最终确定的测验、问卷和 JSON/CSV 导出。

当前实验没有 `Self Generated` 对照条件，也不在实验运行时调用 Stable Diffusion 生成图片。
...
- `ExperimentSessionExport`: 完整会话导出
...
房间中的单词图片默认全部隐藏。参与者进入当前语音路线锚点约 5.5 米范围时，只显示该锚点的图片；看向图片后自动视为已查看。图片会向观察者方向偏移并以前景 UI 方式渲染，避免与墙面或家具穿模。未启用语音路线时，则只显示参与者附近最近的一个图片。
...
VR HMD 模式会在 world-space 学习面板显示当前正在播放的引导语或故事片段字幕。现有的 `Replay Voice` 按钮保持不变。
